...
function InvoiceForm({ refetch }) {
  ...
  const { refetch: refetchProduct } = useGet(
    `/api/GetProductList/${currency}`,
    ["products", currency],
    (data) => {
      setProductsList(data);
      setDefaultProductsList(data);
    }
  );

  useGet(
    `/api/GetCustomers/${selectedBranch?.code}`,
    ["customers", selectedBranch?.code],
    (data) => {
  ...
  useGet("/api/GetCurrency", ["currencies"], (data) => {
    setCurrencies(data);
  });
  ...
  const refetchProductList = async () => {
    try {
      ...
      responseData?.data.length === 0
        ? setProductsList([])
        : setProductsList(responseData?.data || []);
    } catch (error) {}
  };
  ...
  const { refetch: refetchTaxScheme } = useCustomQueryById(
    `/api/GetVatAndLeviesByScheme/${schemeDate}/${
      selectedBranch?.taxScheme || 0
    }`,
    "taxScheme",
    formData?.date,
    onSuccess
    ...
    if (item.itemName === "") {
      ...
      setGridData((gridData) => [
        ...gridData,
        {
          ...
          otherLeviesType: item.otherLevies,
        },
      ]);
  ...
  const checkIfRatesExist = async (currency, date) => {
    ...
    const request = await axios.get(
      `/api/checkIfRatesExist/${selectedBranch?.code}/${currency}/${
    ...
    if (request) {
      ...
      if (data.length > 0) {
        ...
        setCurrency("GHS");
        toast.warning(
          `There are no exchange rates set for ${new Date(
            new Date(formData.date).toISOString().split("T")[0]
          ).toLocaleDateString("en-US", {
            month: "long",
            day: "2-digit",
            year: "numeric",
          })}. Please ensure the rate is set before you can proceed.`
        );
        // setTimeout(() => {
  ...
  const refetchPOItems = async () => {
    ...
    try {
      const request = await axios.get(
        `/api/GetPOItems/${formData?.pon}/${formData?.identity}`
      );
      ...
      if (request?.data) {
        ...
        for (let i = 0; i < request.data.items.length; i++) {
          ...
          const getItem = defaultProductsList.find(
            (x) => x?.code === item?.itemCode
          );
          ...
          if (getItem != undefined) {
            ...
            if (getItem.otherLevies === "NON") {
              ...
              csttourism = (cstRate / 100) * obj.price * obj.quantity;
            } else if (getItem.otherLevies === "TRSM") {
              csttourism = (tourismRate / 100) * obj.price * obj.quantity;
            }

            gridItem = getPayableAmount({ ...obj }, discount, poTaxes);
            setGridData((gridData) => [
              ...gridData,
              {
                ...gridItem,
                ...
                <Row style={{ marginBottom: 10 }}>
                  ...
                  <Col lg="6">
                    ...
                    {formData.discountType === "general" ? (
                      <Badge
                        style={{ fontSize: 9, float: "right" }}
                        className="mt-1"
                        color="primary"
                      >
                        Discount Amount:{" "}
                        {moneyInTxt(formData?.discount, "en", 2) || 0}
                      </Badge>
                    ) : null}
                  </Col>
                </Row>
