...
const Index = () => {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          console.log({
            row,
            gh: row?.original?.signatureStatus?.toUpperCase(),
          });
          return (
            <Button
              ...
              onClick={(e) => {
                ...
                getPrintPDF(
                  row.original.id,
                  row?.original?.signatureStatus?.toUpperCase()
                );
  ...
  const getPrintPDF = async (invoiceNo, isSuccess) => {
    ...
    setPdfData("");
    setShowReport(false);
    //https://localhost:3000/api/GetCompanyDetailsByCompanyId
  ...
  useEffect(() => {
    if (selectedInvoiceNo && printType) {
      getPrintPDF(selectedRow, rowData?.signatureStatus?.toUpperCase());
    }
  ...
  const removeIvoice = async () => {
    try {
      ...
      const response = await axios.post("/api/RemoveInvoice", {
        id: rowData?.id,
  ...
  const retryIvoice = async () => {
    try {
      setIsPostLoading(true);
      const response = await axios.post("/api/RetryInvoice", {
        id: rowData?.id,
        invoiceNumber: rowData?.invoiceNo,
      });
  ...
  return (
    <>
      ...
      <RetryPrompt
        isRetryLoading={isPostLoading}
        isRemoveLoading={isPostLoading}
        handleClose={handleClosePrompt}
        confirmHandeler={confirmHandeler}
        cancelHandeler={cancelHandeler}
        title="No Signature Invoice"
        showPrompt={showPrompt}
        setShowPrompt={setShowPrompt}
        message="Sorry, this invoice is denied signature. Would you like to retry obtaining the signature or remove the invoice?"
      />
              ...
              <Card className="shadow">
                ...
                <div style={styles.body}>
                  <EvatTable
                    ...
                    pdfData={pdfData}
                    message={invoiceQuery.length === 0 ? "" : message}
                    sortKey="date"
                  />
                </div>
